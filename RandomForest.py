...
for directory_path in glob.glob("D:/ProjectDataset/OwnDataset/Hibiscus/Train/*"):
...
for directory_path in glob.glob("D:/ProjectDataset/OwnDataset/Hibiscus/Test/*"):
...
# Import the necessary libraries for the Random Forest classifier
...
x_train, x_test, y_train, y_test = train_test_split(TrainImages, TrainLabelsEncoded, test_size=0.20, random_state=42)
# Flatten the images from 128x128 to 16384 because at first we were not able to 
...
# Get random indices for 9 images
random_indices = np.random.choice(len(x_test), size=9, replace=False)

for i, idx in enumerate(random_indices):
    # Make prediction
    prediction = rf_classifier.predict([x_test_flat[idx]])
